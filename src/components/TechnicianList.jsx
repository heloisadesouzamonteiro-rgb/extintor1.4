...
export default function TechnicianList({ companyId, refreshToken = 0 }) {
  ...
  useEffect(() => {
    async function fetchTechnicians() {
      ...
      const { data, error: queryError } = await supabase.functions.invoke("list-technicians", {
        body: {
          search: search.trim(),
        },
      });
  ...
  async function handleResendConfirmation(technician) {
    ...
    const { data, error: resendError } = await supabase.functions.invoke("resend-technician-confirmation", {
      body: {
